...
class NeuralField(nn.Module):
    ...
    def forward_fitting(self, x, code, smpl_V, return_h=False, f=None):
        """Forward pass through the network with a latent code input.
            Instead of a code_idx which we use to get feats from the codebook, 
            here we directly get the Latent code tensor of shape [1, n_vertices, c_dim].
            This is used by Evaluator.fitting_2D to fit the codebook given an 2D img.
            Args:
                x (torch.FloatTensor): Coordinate tensor of shape [1, N, 3]
                code (torch.FloatTensor): Instead of a code_idx which we use to get feats from the codebook, 
                    here we directly get the Latent code tensor of shape [1, n_vertices, c_dim]
